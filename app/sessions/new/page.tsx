...
export default function NewSessionPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <button onClick={() => router.back()} className="text-sm text-slate-500 hover:text-slate-700 mb-3 flex items-center gap-1">← Kembali</button>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 space-y-4">
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Lokasi <span className="text-slate-300">(opsional)</span></label>
            <input type="text" value={location} onChange={(e) => setLocation(e.target.value)} placeholder="e.g. GOR Mawar..."
              className="w-full border border-slate-300 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary-500" />
          </div>
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wide block mb-1.5">Catatan <span className="text-slate-300">(opsional)</span></label>
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-slate-100 space-y-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {newPlayerNames.map((name) => (
                  <span key={name} className="inline-flex items-center gap-1 bg-shuttle-400/20 text-yellow-800 text-sm px-3 py-1 rounded-full font-medium">
                    ✨ {name}
          ...
          <p className="text-xs text-slate-400">💡 Nama baru akan otomatis ditambahkan ke database.</p>
        </div>
