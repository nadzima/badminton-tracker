...
const navItems = [
  { href: "/", label: "Home", icon: (active: boolean) => (
    <svg className={`w-5 h-5 ${active ? "text-primary-600" : "text-slate-400"}`} fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 1.5} viewBox="0 0 24 24">
    ...
    <>
      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-white border-b border-slate-100">
        <div className="max-w-3xl mx-auto px-4 h-12 flex items-center">
          <span className="font-bold text-slate-800 tracking-tight">Shuttle Track</span>
        </div>
      </header>
        ...
        <div className="max-w-3xl mx-auto flex">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                {item.icon(active)}
                <span className={`text-xs font-medium ${active ? "text-primary-600" : "text-slate-400"}`}>
                  {item.label}
                </span>
              </Link>
            );
