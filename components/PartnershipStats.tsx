        ...
        <tbody className="divide-y divide-slate-50">
          {shown.map((p, i) => (
            <tr key={`${p.player1.id}-${p.player2.id}`}>
              <td className="py-2.5 text-slate-400 text-xs w-5">{i + 1}</td>
              <td className="py-2.5 pr-3">
                <p className="font-medium text-slate-800 text-xs leading-tight">
                  {p.player1.name}
                </p>
                <p className="text-slate-500 text-xs">{p.player2.name}</p>
              </td>
              <td className="py-2.5 text-right text-slate-500 text-xs">{p.matches}</td>
